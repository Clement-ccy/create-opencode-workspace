...
Tags:
  - [标签1]
  - [标签2]
Area: [领域，例如 Deepdive, Tutorial, Review, Practice Note]
Role: Writer
Rate: [1到5的数字，默认可填 5]
...
Platform:
  - Article
  - [其他分发平台，如 Social Media, Video 等]
Link: "[本内容的预期路由链接，例如 /blogs/generative-models-deep-dive]"
Theme: [颜色主题，如 orange, blue, emerald, purple, pink, teal 等]
cover: [预期的封面图片相对路径或占位符，例如 ./generative-models-deep-dive.jpg]
---
```
...
2. **Category (Select)**: 对应 Notion 的 Category 属性（首字母大写）。通常根据内容类型填写，如 `Blogs`。如果是其他类别，也可以是 `Plog`等。
3. **Description (Text)**: 简短的摘要，用于 SEO 或列表预览。必须用双引号包裹。
4. **Tags (Multi-select)**: 文章关键词标签，尽量使用英文，每个标签作为列表的一项。
5. **Area (Select)**: 文章所属的深度领域。例如 `Deepdive`, `Tutorial`, `Review`, `Practice Note`, `Resource`。
...
10. **Platform (Multi-select)**: 预期发布平台。主博客填 `Article`，如果有分发填 `Social Media` 、 `Video` 、 `Audio`。
11. **Link (URL/Text)**: 路由名称的路径。全小写，用短横线连字符连接。例如 `/blogs/your-article-name`。这个名称必须与所在的文件夹 `[name]` 保持一致。
